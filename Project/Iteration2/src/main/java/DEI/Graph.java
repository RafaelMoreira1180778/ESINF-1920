...
public class Graph<V, E> implements GraphInterface<V, E> {
    ...
    /**
     * @param directed Constructs an empty graph (either undirected or directed)
     */
    ...
    public Iterable<Edge<V, E>> edges() {
        LinkedList<Edge<V, E>> l = new LinkedList<>();
        for (V v : vertices.keySet()) {
            for (V v2 : vertices.get(v).getAllAdjVerts()) {
                l.add(vertices.get(v).getEdge(v2));
            }
        }
        return l;
    }
    ...
    public Iterable<Edge<V, E>> incomingEdges(V vert) {
        ...
        for (V v : vertices.keySet()) {
            for (Edge<V, E> v2 : vertices.get(v).getAllOutEdges()) {
                if (v2.getVDest().equals(vert)) {
                    l.add(v2);
                }
            }
        }
    ...
    public String toString() {
        String s = "";
        if (numVert == 0) {
            s = "\nDEI.Graph not defined!!";
        } else {
            s = "DEI.Graph: " + numVert + " vertices, " + numEdge + " edges\n";
            for (Vertex<V, E> vert : vertices.values())
                s += vert + "\n";
        }
        return s;

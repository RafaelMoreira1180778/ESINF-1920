...
public class TREE<E extends Comparable<E>> extends BST<E> {
    ...
    public boolean isLeaf(E element) {
        BST.Node<E> node = find(element, root);
    ...
    private void ascSubtree(BST.Node<E> node, List<E> snapshot) {
        if (node == null) return;
    ...
    private void desSubtree(BST.Node<E> node, List<E> snapshot) {
        if (node == null) return;
        ascSubtree(node.getRight(), snapshot);
        snapshot.add(node.getElement());
        ascSubtree(node.getLeft(), snapshot);
    ...
    private BST.Node<E> copyRec(BST.Node<E> node) {
        if (node == null) return node;
        if (isLeaf(node.getElement())) return null;
        return new BST.Node(node.getElement(), copyRec(node.getLeft()), copyRec(node.getRight()));
    }
    ...
    private void numNodesByLevel(BST.Node<E> node, int[] result, int level) {
        if (node == null) return;
        result[level]++;

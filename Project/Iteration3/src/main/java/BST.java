...
public class BST<E extends Comparable<E>> implements BSTInterface<E> {
    ...
    protected E smallestElement(Node<E> node) {
        ...
        E leftMin, rightMin;
        E min = node.getElement();
        if (node.getLeft() != null) {
            leftMin = smallestElement(node.getLeft());
            if (leftMin.compareTo(min) < 0) min = leftMin;
        }
        if (node.getRight() != null) {
            rightMin = smallestElement(node.getRight());
    ...
    protected Node<E> find(E element, Node<E> node) {
        ...
        if (element.compareTo(node.getElement()) < 0) return find(element, node.left);
    ...
    public Iterable<E> inOrder() {
        ...
        if (root != null) inOrderSubtree(root, snapshot);
        return snapshot;
    }
    ...
    public Iterable<E> preOrder() {
        List<E> snapshot = new ArrayList<>();
        if (root != null) preOrderSubtree(root, snapshot);
        return snapshot;
    }
    ...
    public Iterable<E> posOrder() {
        List<E> snapshot = new ArrayList<>();
        if (root != null) posOrderSubtree(root, snapshot);
        return snapshot;
    }
    ...
    public Map<Integer, List<E>> nodesByLevel() {
        ...
        int level = 0;
        if (root != null) processBstByLevel(root, result, level);
        return result;
    }
